...
// Konfigurasi CORS agar React bisa menembak ke API Bun ini
const corsHeaders = {
...
// Fungsi Helper untuk merespons dengan JSON
  ...
  async fetch(req) {
      ...
      try {
        ...
        jcTab.on("request", async (request) => {
          ...
          if (request.url().includes("/api/users/getSelf")) {
            ...
            await mainTab.bringToFront(); // Pindah layar otomatis
          }
        });

        await jcTab.goto("https://console.jumpcloud.com/login");
        // Langsung return sukses ke React agar tidak timeout!
    ...
    if (url.pathname === "/api/connect-sipgn" && req.method === "GET") {
      try {
        ...
        sipgnTab.on("request", async (request) => {
          ...
          // TODO: Sesuaikan path ini dengan URL API Dashboard SIPGN
          const targetApiUrl =
          ...
          if (request.url().includes(targetApiUrl)) {
            ...
            if (authHeader && authHeader.toLowerCase().startsWith("bearer ")) {
              ...
              console.log("✅ Bearer Token SIPGN didapatkan!");
              await mainTab.bringToFront(); // Pindah layar otomatis
            }
          }
        });

        // TODO: Sesuaikan dengan URL halaman Login SIPGN yang asli
    ...
    if (url.pathname === "/api/action-sipgn" && req.method === "POST") {
      ...
      try {
        ...
        const response = await fetch(
          `https://sipgn-api.bgn.go.id/api/v1/sipai-dashboard/sppg-connected?has_streaming=true&search=${id}`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
    ...
    // // 2. Kalau user butuh file JS/CSS dari Vite
...
// =====================================================================
async function init() {
  console.log("✅ Backend API berjalan di http://localhost:4000");
  try {
    ...
    // Pastikan Vite React sedang berjalan di port 5173
    await mainTab.goto("http://localhost:5173");

    // await mainTab.goto("http://localhost:4000");
    console.log("✅ Frontend UI berhasil dibuka di browser.");
  } catch (error) {
